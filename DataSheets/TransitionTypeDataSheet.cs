...
namespace SyncroSim.STSim
{
    ...
    internal class TransitionTypeDataSheet : DataSheet
    {
        ...
        private void OnTTGRowsAdded(object sender, DataSheetRowEventArgs e)
        {
            ...
            using (DataStore store = this.Library.CreateDataStore())
            {
                foreach (DataRow dr in this.m_TTData.Rows)
                {
                    ...
                    int ttid = Convert.ToInt32(dr[IdColName]);
                ...
                if (MissingTypeGroups.Count > 0)
                {
                    ...
                    {
                        string[] s = Key.Split('-');
                        this.CreateTransitionTypeGroup(int.Parse(s[0]), int.Parse(s[1]));
                    }
        ...
        protected override void OnRowsAdded(object sender, DataSheetRowEventArgs e)
        {
            ...
            using (DataStore store = this.Library.CreateDataStore())
            {
                foreach (DataRow dr in this.m_TTData.Rows)
                {
                    ...
                    int ThisId = Convert.ToInt32(dr[IdColName]);
                    string AutoGroupName = GetAutoGeneratedGroupName(dr);
        ...
        public override void DeleteRows(IEnumerable<DataRow> rows)
        {
            ...
            {
                this.m_TGDataSheet.DeleteRows(DeleteRows);
            }
        ...
        protected override void OnModifyingRows(object sender, DataSheetRowEventArgs e)
        {
            ...
            foreach (DataRow dr in this.m_TTData.Rows)
            {
                ...
                int TypeId = Convert.ToInt32(dr[IdColName]);
                string TypeName = Convert.ToString(dr[Strings.DATASHEET_NAME_COLUMN_NAME, DataRowVersion.Current]);
        ...
        protected override void OnRowsModified(object sender, DataSheetRowEventArgs e)
        {
            ...
            foreach (DataRow dr in this.m_TTData.Rows)
            {
                ...
                int id = Convert.ToInt32(dr[IdColName]);
                ...
                string NewName = Convert.ToString(dr[Strings.DATASHEET_NAME_COLUMN_NAME]);
            ...
            if (ModifyRows.Count > 0)
            {
                ...
                {
                    string OldName = this.m_PrevNames[Convert.ToInt32(dr[IdColName])];
                    string NewName = Convert.ToString(dr[Strings.DATASHEET_NAME_COLUMN_NAME]);
        ...
        private Dictionary<string, DataRow> CreateTGRowDictionary()
        {
            ...
            foreach (DataRow dr in this.m_TGData.Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {
                    d.Add(Convert.ToString(dr[Strings.DATASHEET_NAME_COLUMN_NAME]), dr);
                }
            }
        ...
        private Dictionary<string, bool> CreateTTG_ID_Dictionary()
        {
            ...
            foreach (DataRow dr in this.m_TTGData.Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {
                    string k = Create_TT_TG_Key(Convert.ToInt32(
                        dr[Strings.DATASHEET_TRANSITION_TYPE_GROUP_TYPE_COLUMN_NAME]), 
                        Convert.ToInt32(dr[Strings.DATASHEET_TRANSITION_TYPE_GROUP_GROUP_COLUMN_NAME]));
        ...
        {
            return GetAutoGeneratedGroupName(Convert.ToString(dr[Strings.DATASHEET_NAME_COLUMN_NAME]));
        }
