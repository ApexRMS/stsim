...
Class TransitionAttributeRasterMap
    Inherits StochasticTimeExportTransformer

    ReadOnly fileFilterRegex As String = String.Format(CultureInfo.CurrentCulture, FILE_FILTER_ID_REGEX, SPATIAL_MAP_TRANSITION_ATTRIBUTE_VARIABLE_PREFIX)
    ...
    Private Function CreateExportFilename(sourceFileName As String) As String
        Return CreateTransAttrExportFilename(sourceFileName)
    End Function

    ''' <summary>
    ''' Create an Transition Attribute filename using the export filename convention. This involves replacing the Id with the Transition 
    ''' Attribute Name
    ''' </summary>
    ...
    ''' <remarks>Internal file convention is Itx-Tsy-ta-z.tif. External convention is ...Ta-TransitionAttributeName.tif</remarks>
    Private Function CreateTransAttrExportFilename(ByVal filename As String) As String
        ...
        If Not m.Success Or m.Groups.Count <> 4 Then
            ...
            Debug.Assert(False, "Error parsing the Transition Attribute internal filename.")
            Return filename
        End If
        ...
        Return String.Format(CultureInfo.InvariantCulture, "{0}{1}-{2}.{3}", m.Groups(1), SPATIAL_MAP_EXPORT_TRANSITION_ATTRIBUTE_VARIABLE_PREFIX, name, m.Groups(3))
